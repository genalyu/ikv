...
streaming cache 的 committed 写入是事务性的：单层 attention/backend 报错会恢复该层覆盖或淘汰的槽位；后续 layer 或输出 head 报错会恢复本次 model forward 已写入的所有层。server 的一次 grounding 在编码和追加 KV 之前开始外层事务，并把 video pass 与 action pass 都放在其中，因此 action pass 失败不会留下只有 video 新 KV 已提交的半状态。真实观测到达不再清理已有 prediction KV。普通/冷启动 imagination 同样把最终 video 与 action 的 `update_cache=1` 写入放在一个请求事务里，事务一直覆盖到 action postprocess 成功；因此不支持的输出格式等晚期错误也不会留下半个预测 chunk。回滚同时覆盖 K、V、slot id、occupancy、predicted 标志和独立 semantic sidecar；实现只暂存本次触及的槽位，而不会复制整个 cache pool。
...
- server 当前在真实 observation 重新 grounding 时追加 observed KV，不再自动清除或置零 prediction KV；仍可能因容量淘汰或 episode reset 移除。按物体进行语义替换不是此旧版 sparse 实验的在线策略。当前完整 RGB 与全局容量策略见 [INDEXED_KV.md](INDEXED_KV.md)。
