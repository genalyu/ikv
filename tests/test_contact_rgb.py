...
def test_neoforce_native_rgb_without_mutating_policy(monkeypatch, history, swapped, active):
    server = dense_server()
    cfg=server.job_config
    cfg.kv_neoforce_online=True
    cfg.kv_index_input_rb_swapped=swapped
    ...
    def build(got,**kw):
        ...
        np.testing.assert_array_equal(actual,image[...,::-1] if swapped else image)
